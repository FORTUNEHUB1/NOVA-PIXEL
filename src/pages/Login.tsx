...
export default function Login() {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Smoothly navigate to admin dashboard
      navigate('/admin');
    } catch (err: any) {
      console.error(err);
      ...
      >
        ...
        <form onSubmit={handleEmailSignIn} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 bg-[#111] dark:bg-white text-white dark:text-black rounded-xl font-black uppercase tracking-widest hover:bg-[#FA1594] dark:hover:bg-[#FA1594] dark:hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="mt-8 pt-8 border-t border-gray-200 dark:border-zinc-800 text-center">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-4 bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl flex items-center justify-center gap-3 hover:bg-gray-50 dark:hover:bg-zinc-700 transition-colors disabled:opacity-50"
          >
